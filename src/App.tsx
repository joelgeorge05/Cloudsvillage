                ...
                <div className="absolute bottom-[-3rem] left-0 right-0 flex justify-center gap-3">
                  {lightboxImages.map((_, idx) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all duration-300 ${idx === lightboxIndex ? 'w-10 bg-brand-cyan shadow-[0_0_10px_rgba(0, 180, 216,0.8)]' : 'w-3 bg-white/30 hover:bg-white/60'}`}
                    />
                  ))}
                </div>
              </motion.div>
